...
public class Starter {
		...
		try {
			...
			if (args[0].equals("start")) {
				if (args.length == 2) {
					...
					int step = BackupUtilities
							.detectLastSucsessfulStep(workingFolder);
					LoggerFactory.getInstance().println(
							"Last full step detected: " + step
									+ ". Continue to work...");
					startAll(step);
				} else {
	...
	private static void startAll(int lastSuccessfulStep) throws IOException {
		...
		if (lastSuccessfulStep < 2) {
			ExperimentsPreparer.start(new String[] {
					workingFolder + FIRST_STEP_FILE,
					workingFolder + SECOND_STEP_FOLDER, NUM_OF_EXPERIMENTS });
		...
		if (lastSuccessfulStep < 3) {
			...
			for (File folder : f.listFiles()) {
				if (folder.isDirectory()) {
					FeatureEvaluater.start(new String[] {
							folder.getAbsolutePath() + TRAINING_FEATURES,
							folder.getAbsolutePath() + TRAINING_CLASSES,
		...
		if (lastSuccessfulStep < 4) {
			...
			for (File folder : inFolder.listFiles()) {
				if (folder.isDirectory()) {
					String outFolder = f.getAbsolutePath() + "/"
							+ folder.getName() + "/";
					Voter.start(new String[] { folder.getAbsolutePath(), "4",
							outFolder, Voter.WPCA });
					// UseVoting.start(new String[] { folder.getAbsolutePath(),
					// "4", outFolder, UseVoting.PCA_BASED });
					/*
					 * SelectFeatures.start(new String[] {
					 * folder.getAbsolutePath(), "4", outFolder });
					 * //SelectFeatures.start(new String[] {
					 * folder.getAbsolutePath(), "2", outFolder });
					 * SelectFeatures.start(new String[] {
					 * folder.getAbsolutePath(), "1", outFolder });
					 * //UseVoting.start(new String[] {
					 * folder.getAbsolutePath(), "2", outFolder, UseVoting.BORDA
					 * }); UseVoting.start(new String[] {
					 * folder.getAbsolutePath(), "4", outFolder, UseVoting.BORDA
					 * }); //UseVoting.start(new String[] {
					 * folder.getAbsolutePath(), "2", outFolder,
					 * UseVoting.CONDORCET }); UseVoting.start(new String[] {
					 * folder.getAbsolutePath(), "4", outFolder,
					 * UseVoting.CONDORCET }); UseVoting.start(new String[]
					 * {folder.getAbsolutePath(), "4", outFolder,
					 * UseVoting.MARKOV });
		...
		if (lastSuccessfulStep < 5) {
			...
			for (File folder : inFolder.listFiles()) {
				if (folder.isDirectory()) {
					...
					File outSubFolder = new File(workingFolder
							+ FIFTH_STEP_FOLDER + "/" + folder.getName());
					outSubFolder.mkdir();

					for (String method : folder.list()) {
						File outMethodFolder = new File(
								outSubFolder.getAbsolutePath() + "/matrix_"
										+ method);

						outMethodFolder.mkdir();
						String secondSubFolder = workingFolder
								+ SECOND_STEP_FOLDER + "/" + folder.getName();

						NewMatrixFromFeaturesCreator.start(new String[] {
								secondSubFolder + TRAINING_FEATURES,
								secondSubFolder + TEST_FEATURES,
								workingFolder + FOURTH_STEP_FOLDER + "/"
										+ folder.getName() + "/" + method,
								outMethodFolder.getAbsolutePath() });
						// TODO may be move to individual class
						Files.copy(new File(secondSubFolder + TRAINING_CLASSES)
								.toPath(),
								new File(outMethodFolder.getAbsolutePath()
										+ TRAINING_CLASSES).toPath());

						Files.copy(new File(secondSubFolder + TEST_CLASSES)
								.toPath(),
								new File(outMethodFolder.getAbsolutePath()
										+ TEST_CLASSES).toPath());

						MatrixUtilities.createWekaMatrix(new File(
								outMethodFolder.getAbsolutePath()
										+ TRAINING_FEATURES), new File(
								outMethodFolder.getAbsolutePath()
										+ TRAINING_CLASSES), new File(
								outMethodFolder.getAbsolutePath()
										+ WEKA_TRAINING));

						MatrixUtilities.createWekaMatrix(new File(
								outMethodFolder.getAbsolutePath()
										+ TEST_FEATURES), new File(
								outMethodFolder.getAbsolutePath()
										+ TEST_CLASSES), new File(
								outMethodFolder.getAbsolutePath() + WEKA_TEST));
					}
				}
		...
		if (lastSuccessfulStep < 6) {
			...
			for (File folder : inFolder.listFiles()) {
				if (folder.isDirectory()) {
					LoggerFactory.getInstance().println(
							"Folder " + folder.getName());
					File outSubFolder = new File(f.getAbsolutePath() + "/"
							+ folder.getName());
					outSubFolder.mkdir();
					File inSubFolder = new File(inFolder.getAbsolutePath()
							+ "/" + folder.getName());
					for (String method : inSubFolder.list()) {
						// TODO may be move to individual class
						SVM classifier = new SVM();//<==== set classifier
						classifier.teachClassifier(new File(inFolder
								.getAbsolutePath()
								+ "/"
								+ folder.getName()
								+ "/" + method + WEKA_TRAINING));
						classifier
								.writeClassifier(new File(f.getAbsolutePath()
										+ "/" + folder.getName() + CLASSIFIER
										+ method));
					}
				}
				LoggerFactory.getInstance().println(" is done\n");
			...
			for (File folder : inFolder.listFiles()) {
				if (folder.isDirectory()) {
					...
					File inSubFolder = new File(inFolder.getAbsolutePath()
							+ "/" + folder.getName());
					for (String method : inSubFolder.list()) {
						ClassifierEvaluater.start(new String[] {
								inFolder.getAbsolutePath() + "/"
										+ folder.getName() + "/" + method,
								workingFolder + SIXTH_STEP_FOLDER + "/"
										+ folder.getName() + CLASSIFIER
										+ method,
								f.getAbsolutePath() + "/" + folder.getName()
										+ "/" + method + "/" });
					}
				}
				LoggerFactory.getInstance().println(" is done\n");
